...
const checkElForProps = (el, props = []) => {
  return props.reduce((a, p) => {
    try {
      a[p] = el.elements.find(
        el => el.attributes.identifier.toLowerCase() === p,
      ).elements[0].text
    } catch (e) {
      false
    }
...
const findReplacementNodes = xml => {
  ...
  const recurse = array => {
    array.forEach(el => {
      ...
      if (kind === "image") {
        const data = checkElForProps(el, [
          "image",
          "url",
          "title",
          "description",
        ])
        replacements.push({ type: "image", ...data })
        return
      }
      if (kind === "link") {
        const data = checkElForProps(el, ["url", "title"])
        replacements.push({ type: "link", ...data })
        replacements.push({ type: "linkEnd", ...data })
      }
      if (el.elements) {
        recurse(el.elements)
      }
...
const mapNodesToHtml = replacements => {
  return replacements.map(r => {
    if (r.type === "image") {
      let el = ""
      el += `![${r.description ? r.description : ""}](${
        r.image ? r.image : ""
      }${r.title ? ` "${r.title}"` : ""})`
      r.url && (el = `[${el}](${r.url})`)
      return el
